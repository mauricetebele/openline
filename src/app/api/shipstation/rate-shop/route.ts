...
export async function POST(req: NextRequest) {
  ...
  if (isAmazonOrder && body.amazonOrderId) {
    try {
      ...
      // Secondary, redundant source — the primary Amazon Buy Shipping rates come
      // from ShipStation's amazon_buy_shipping carrier (which returns the full
      // list incl. UPS Ground). Just log if this supplementary MFN call fails.
      const msg = err instanceof Error ? err.message : String(err)
