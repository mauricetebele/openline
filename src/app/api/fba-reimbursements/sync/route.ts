/**
 * POST /api/fba-reimbursements/sync — Trigger manual FBA reimbursements sync
 * GET  /api/fba-reimbursements/sync?jobId=X — Poll job status
 */
...
export async function POST(req: NextRequest) {
  ...
  waitUntil(
    syncFbaReimbursements(accountId, job.id, start, end).catch(async (err: unknown) => {
      const message = err instanceof Error ? err.message : String(err)
      console.error('[FbaReimbursements] Sync job failed:', message)
