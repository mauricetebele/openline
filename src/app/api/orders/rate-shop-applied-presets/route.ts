        ...
        const processOrder = async (order: typeof orders[number]) => {
          ...
          try {
            ...
            if (fedexDirectOnly && fedexCreds) {
              ...
              // returned rate INCLUDES the signature surcharge (otherwise FedEx
              // quotes a no-signature rate that's too cheap).
              ...
              for (let attempt = 0; attempt < 3; attempt++) {
                ...
                if (attempt < 2 && firstErr && TRANSIENT_RE.test(firstErr)) {
                  console.log('[rate-shop-presets] V2 transient retry %d: %s', attempt + 1, firstErr)
                  continue
                }
                break
              }
