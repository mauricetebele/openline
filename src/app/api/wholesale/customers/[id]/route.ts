...
export async function GET(
  ...
  // Open balance = order balances - unallocated credit memos - unallocated payments
  const [orderBal, cmTotal, cmAllocated, pmtUnallocated] = await Promise.all([
  ...
  const unallocatedPayments = Number(pmtUnallocated._sum.unallocated ?? 0)
  const openBalance = Number(orderBal._sum.balance ?? 0) - unallocatedCredits - unallocatedPayments
