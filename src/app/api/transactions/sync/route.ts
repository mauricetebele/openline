/**
 * POST /api/transactions/sync — Trigger Amazon transaction sync
 * GET  /api/transactions/sync?jobId=X — Poll job status
 */
...
export async function POST(req: NextRequest) {
  ...
  waitUntil(
    syncAmazonTransactions(accountId, start, end, job.id).catch(async (err: unknown) => {
      const message = err instanceof Error ? err.message : String(err)
      console.error('[sync-transactions] Sync job failed:', message)
