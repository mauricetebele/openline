/**
 * POST /api/fba-refunds/sync — Trigger manual FBA refund sync
 * GET  /api/fba-refunds/sync?jobId=X — Poll job status
 */
...
export async function POST(req: NextRequest) {
  ...
  waitUntil(
    syncFbaRefunds(accountId, start, end, job.id).catch(async (err: unknown) => {
      const message = err instanceof Error ? err.message : String(err)
      console.error('[FbaRefunds] Sync job failed:', message)
