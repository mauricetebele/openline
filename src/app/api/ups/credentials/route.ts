/**
 * GET  /api/ups/credentials  — returns whether credentials are configured (no secrets)
 * POST /api/ups/credentials  — save UPS Client ID, Secret, and Account Number (encrypted)
 */
...
export async function POST(req: NextRequest) {
  ...
  const existing = await prisma.upsCredential.findFirst({ where: { isActive: true } })
  if (existing) {
    await prisma.upsCredential.update({ where: { id: existing.id }, data })
  } else {
    await prisma.upsCredential.create({ data: { ...data, clientIdEnc: data.clientIdEnc, clientSecretEnc: data.clientSecretEnc } })
  }
