...
interface ManifestRow {
  id: string
  source: 'marketplace' | 'wholesale' | 'vendorRMA' | 'fba' | 'repair' | 'accessorial'
...
export default function ShippingManifest() {
  ...
  // Filters
  const [channelFilter, setChannelFilter] = useState<'all' | 'marketplace' | 'wholesale' | 'vendorRMA' | 'fba' | 'repair' | 'accessorial'>('all')
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex flex-wrap items-center gap-3 px-6 py-3 border-b bg-white dark:bg-gray-900 dark:border-gray-700 shrink-0">
        ...
        <select
          value={channelFilter}
          onChange={(e) => { setChannelFilter(e.target.value as 'all' | 'marketplace' | 'wholesale' | 'vendorRMA' | 'fba' | 'repair' | 'accessorial'); setPage(0) }}
                  ...
                  >
                    <td className="px-3 py-1.5">
                      {row.source === 'wholesale' ? (
                        ...
                        <span className="inline-flex px-1.5 py-0.5 rounded text-[10px] font-medium bg-rose-100 text-rose-700 border border-rose-200">REPAIR</span>
                      ) : row.source === 'accessorial' ? (
                        <span className="inline-flex px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-100 text-amber-800 border border-amber-300">ACC</span>
                      ) : (
                    ...
                    <td className="px-3 py-1.5 font-mono">
                      {row.source === 'wholesale' ? (
                        ...
                        <span>
                          <span className="text-rose-600 font-medium">{row.orderRef}</span>
                          {row.customerName && <span className="text-gray-400 ml-1.5 font-sans">{row.customerName}</span>}
                        </span>
                      ) : row.source === 'accessorial' ? (
                        <span>
                          {row.olmNumber ? <span className="text-gray-500 mr-1.5">OLM-{row.olmNumber}</span> : null}
                          <span className="text-amber-700 font-medium">{row.amazonOrderId}</span>
                        </span>
                      ) : (
                        <span>
                          {row.olmNumber ? <span className="text-gray-500 mr-1.5">OLM-{row.olmNumber}</span> : null}
