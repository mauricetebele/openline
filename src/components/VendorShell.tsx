...
function VendorHeader() {
  ...
  const links = [
    { href: '/vendor/inventory', label: 'Inventory', show: true },
    { href: '/vendor/purchase-orders', label: 'Purchase Orders', show: user?.canViewPurchaseOrders },
    { href: '/vendor/oli', label: 'OLI', show: user?.canAccessOli },
  ]
  ...
  return (
    <header className="sticky top-0 z-30 bg-gradient-to-b from-amazon-dark to-[#1a1f2e] text-white shadow-lg">
      <div className="flex items-center gap-3 px-4 h-12">
        ...
        <span className="text-sm font-medium text-gray-300">Vendor Portal</span>
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-xs font-bold text-white uppercase shrink-0 shadow-sm">
              {user?.email?.[0] ?? '?'}
            </div>
...
function VendorShellInner({ children }: { children: React.ReactNode }) {
